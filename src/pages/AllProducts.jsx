...
const AllProducts = ({ user, handleAuthPopup }) => {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const location = useLocation();
  const [selectedProduct, setSelectedProduct] = useState(null);
  // 1. Capture the searched product name from the Navbar click
  const targetProduct = location.state?.searchTerm;

useEffect(() => {
  const fetchAndSort = async () => {
    setLoading(true);
    
    // 1. Fetch products AND categories in one query
    ...
    if (error) {
      ...
      // 2. Apply sorting if a targetProduct exists from search
      if (targetProduct) {
        finalData = [...data].sort((a, b) => {
          const isA = a.name.toLowerCase() === targetProduct.toLowerCase();
          const isB = b.name.toLowerCase() === targetProduct.toLowerCase();
          if (isA) return -1;
          if (isB) return 1;
          return 0;
        });
      ...
      console.log("Products data:", finalData);
    }
    setLoading(false);
  };

  fetchAndSort();
}, [targetProduct]); // Dependency ensures it re-sorts if user searches again
  ...
  if (loading) return <div className="p-20 text-center text-gray-500">Loading Inventory...</div>;

  return (
    <div className="min-h-screen bg-amber-50 dark:bg-gray-900 py-12">
      <div className="container mx-auto px-4">
        <h1 className="text-3xl md:text-5xl font-black text-primary leading-none mb-3 drop-shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {products.map((item) => (
              <div
                ...
                className={`relative bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md border border-gray-100 dark:border-gray-700 hover:shadow-lg transition-all cursor-pointer group
                  ${item.stock_qty <= 0 ? 'opacity-60 grayscale' : ''}
                `}
              >
                {/* Out of Stock Banner */}
                {item.stock_qty <= 0 && (
                  <div className="absolute inset-0 z-10 flex items-center justify-center rounded-xl bg-gray-200/50 dark:bg-gray-900/50">
                    <span className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400 text-xs font-semibold tracking-widest uppercase px-4 py-2 rounded-full shadow">
                      Sold Out
                    </span>
                  </div>
                )}
                ...
                <button
                  onClick={(e) => {
                    ...
                    item.stock_qty > 0 && addToCart(item, user, handleAuthPopup);
                  }}
