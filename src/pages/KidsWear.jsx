...
const KidsWear = ({ user, handleAuthPopup }) => {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedProduct, setSelectedProduct] = useState(null);
  const fetchKidsWear = async () => {
    setLoading(true);
    // Join products with categories and filter for Kids Wear
    const { data, error } = await supabase
      ...
      `)
      .eq('categories.name', 'Kids Wear'); //

    if (error) {
      console.error("Error fetching Kids Wear:", error.message);
    } else {
      setProducts(data || []);
    }
    setLoading(false);
  ...
  if (loading) return <div className="p-20 text-center text-gray-500 text-xl">Loading Kids Wear...</div>;
